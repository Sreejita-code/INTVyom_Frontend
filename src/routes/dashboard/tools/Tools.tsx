...
import {
  callCreateToolEndpoint,
  callDeleteToolEndpoint,
  callGetToolDetailsEndpoint,
  callListToolsEndpoint,
  callUpdateToolEndpoint,
  condenseListToolsResponse,
...
export default function ToolsPage() {
  ...
  const handleSelectTool = async (id: string) => {
    ...
    try {
      ...
      if (ok && json && typeof json === "object" && (json as Record<string, unknown>).data) {
        ...
        setFormData({
          tool_id: d.tool_id as string | undefined,
          tool_name: (d.tool_name as string) || "",
          tool_description: (d.tool_description as string) || "",
          tool_execution_type: (d.tool_execution_type as "webhook" | "static_return") || "webhook",
          tool_execution_config: d.tool_execution_config || {},
          tool_parameters: ((d.tool_parameters as any[]) || []).map((p: any) => ({
            ...p,
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const json =
        mode === "create"
          ? await callCreateToolEndpoint(payload)
          : await callUpdateToolEndpoint(selectedId as string, payload);

      toast({ title: mode === "create" ? "Tool Created" : "Tool Updated" });
      await fetchList();

      if (mode === "create" && json && typeof json === "object" && (json as Record<string, unknown>).data) {
        const createdId = ((json as Record<string, unknown>).data as Record<string, unknown>).tool_id as string | undefined;
        if (createdId) handleSelectTool(createdId);
      }
    } catch (error: any) {
      toast({ variant: "destructive", title: "Error", description: error.message });
    } finally {
